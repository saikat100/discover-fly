...
document.querySelector('.book-now').addEventListener('click', function () {
    ...
    if (!fryingFrom || !fryingTo || !fryingDeparture || (!firstClassTicket && !economyTicket)) {
        ...
        if (firstClassTicket === "") {
			document.querySelector(".user-first-class").textContent = "0 ";
		} else {
			document.querySelector(".user-first-class").textContent = firstClassTicket;
		}
        
        if (economyTicket === "") {
			document.querySelector(".user-economy-class").textContent = "0 ";
		}
        else{
